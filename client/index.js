...
const sendMessage = message => {
  if(backendRSA){
    socket.emit('send-message', { message: backendRSA.encrypt(message).toString('base64'), id: socket.id })
  }
  cleanInput()
}
...
socket.on('security', ({ publicKey }) => {
  ...
  socket.emit('security-response', { publicKey: frontendRSA.exportKey('public'), id: socket.id })
})
...
socket.on('list-messages', (messages) => {
  ...
  if(messages.length > 0){
    // reder every message
    messages.forEach(msg => renderMessage(msg.authorId, frontendRSA.decrypt(msg.message).toString()))
  }
})